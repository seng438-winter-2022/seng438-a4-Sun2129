...
Equivalent mutations are bad as they are typically redundant mutations that occur. An example would be when adding two numbers that do not have any specific requirements needed such as only being positive or negative numbers. An equivalent mutant in that situation would be having only positive numbers and then only negative numbers therefore making it an equivalent class as having two mutations is redundant since both check to make sure that the function works. This is bad since if there are enough equivalent mutants then you can have an abnormally high mutation score and then it would not provide an accurate score as many of the typically bugs that have been killed are a repeated variations of them which is largely due to the fact the equivalent mutants have the same or similar process as the program
